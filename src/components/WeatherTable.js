    ...
    const columns = [
        {
            ...
            filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters, close }) => (
                <div style={{ padding: 8 }} onKeyDown={e => e.stopPropagation()}>
                    <RangePicker
                        showTime={{ format: 'HH' }}
                        value={selectedKeys[0] && selectedKeys[0].length === 2
                    ...
                    <Space>
                        <Button
                            type="primary"
                            onClick={() => {
                                const currentDates = selectedKeys[0] && selectedKeys[0].length === 2
                                    ? [moment(selectedKeys[0][0]), moment(selectedKeys[0][1])]
                                    : null;
            ...
            render: (_, { temperature }) => { 
                ...
                return (
                    <Tag color={color} key={currentTemperature}>
                        {currentTemperature !== undefined && currentTemperature !== null ? currentTemperature : 'N/A'}
                    </Tag>
                );
            },
